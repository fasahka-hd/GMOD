...
function PANEL:UpdateAccessoryPosition(itemTable, ent)
    ...
    if isstring(boneName) and boneName:find("Head") then
        offsetPos = offsetPos + Vector(0, 0, AAS.HeadLift or 0)
    end

    pos = AAS.ConvertVector(pos, offsetPos, ang)
    ang = AAS.ConvertAngle(ang, offsetAng)

    if not ent.AASScale or ent.AASScale != itemTable.scale then
        ent:SetModelScale(itemTable.scale, 0)
        ent.AASScale = itemTable.scale
    end
...
function PANEL:createAccessory(itemTable)
    ...
    timer.Create("aas_refresh_accessories:"..cat, 0.03, 0, function()
