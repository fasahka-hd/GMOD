...
local tmysql_connect = tmysql.Connect or tmysql.initialize;
assert( isfunction( tmysql_connect ), 'TMySQL4: Connect API not found' );
...
function ptmysql.connect( hostname, username, password, database, port, optional_unix_socket_path )
  ...
  obj._db, obj.err   = tmysql_connect( hostname, username, password, database, port, optional_unix_socket_path );
...
function db_mt:setCharset( charset )
  if self._db.SetCharacterSet then
    return self._db:SetCharacterSet( charset );
  end
...
function db_mt:query( sqlstr, cback )
  return self._db:Query( sqlstr, function( results )
    ...
    if not result then
      ptmysql.log( self.database .. ':' .. self.port .. ' - Empty query result' );
      return
    end

    if result.error then
      -- Modern TMySQL4 already tries to reconnect internally. Permanent SQL
      -- errors must not be queued forever (for example duplicate primary keys).
      ptmysql.log( self.database .. ':' .. self.port .. ' - ' .. tostring( result.error ) );
      return
    end

    if cback then cback( result.data or { } ); end
  end );
end
...
-- Polling is handled by lua/autorun/tmysql4.lua.
